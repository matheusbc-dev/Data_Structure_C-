...
public:
    ...
    // Função para realizar a travessia in-order da árvore
    void inOrderTreeWalk(TreeNode* x) {
        if (x != nullptr) {
            inOrderTreeWalk(x->left);    // Visita a subárvore esquerda
            std::cout << x->key << " "; // Visita o nó
            inOrderTreeWalk(x->right);   // Visita a subárvore direita
        }
    }
};
...
int main() {
    ...
    // Realizar travessia in-order e imprimir os nós
