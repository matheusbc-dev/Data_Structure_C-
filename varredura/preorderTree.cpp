...
public:
    ...
    // Função para realizar a travessia pre-order da árvore
    void preOrderTreeWalk(TreeNode* x) {
        if (x != nullptr) {
            std::cout << x->key << " "; // Visita o nó
            preOrderTreeWalk(x->left);   // Visita a subárvore esquerda
            preOrderTreeWalk(x->right);  // Visita a subárvore direita
        }
    }
};
...
int main() {
    ...
    // Realizar travessia pre-order e imprimir os nós
